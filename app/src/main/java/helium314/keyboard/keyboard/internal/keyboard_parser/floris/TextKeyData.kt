            ...
            KeyLabel.PERIOD -> (
                if (
                    params.mId.isAlphabetKeyboard && (
                        (params.mLocaleKeyboardInfos.labelFlags and Key.LABEL_FLAGS_HAS_POPUP_HINT) != 0 ||
                        (params.mId.mMode == KeyboardId.MODE_TEXT && params.mId.getLocale().language == "ar")
                    )
                ) Key.LABEL_FLAGS_HAS_POPUP_HINT else 0
            ) or
